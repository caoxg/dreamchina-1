...
import com.dreamchina.MainActivity;
import com.dreamchina.R;
import com.dreamchina.config.Constants;
...
public class AzkHelper {
	...
	public static void prepareUserAgent(Context context) {
		String versionString = getApplicationVersionString(context);
		String channel = "";
		String deviceId = ((TelephonyManager) context
				.getSystemService(Context.TELEPHONY_SERVICE)).getDeviceId();
		channel = getChannelName(context, "UMENG_CHANNEL");
		sAzkUserAgent = String.format(context.getString(R.string.user_agent),
				Build.VERSION.RELEASE, Constants.APP_NAME, versionString,
				Build.MODEL, channel, deviceId);
		AzkHelper.showLog(sAzkUserAgent);
	}
	...
	public static String getMetaData(Context context, String name) {
		try {
			...
			AzkHelper.showLog("meta", name + ": " + value);
			return value;
	...
	public static void showToast(Context context, String str) {
		...
		toastReference = new WeakReference<Toast>(toast);
		toast.show();
	}
	...
	private static void hideToast() {
		if (toastReference != null) {
			Toast toast = toastReference.get();
			if (toast != null)
				toast.cancel();
		}
	}
	...
	public static void showErrorToast(Activity activity, String retString,
		...
		try {
			...
			AzkHelper.showErrorLog("showErrorToast:" + e.toString());
		}
	}
	...
	public static void launchApp(Context context, Class<?> lauchActivityClass) {
		Intent jumpIntent = new Intent(context, lauchActivityClass);
		jumpIntent.setAction(Intent.ACTION_MAIN);
	...
	public static void showAlert(Activity activity, String message) {
		...
		new AlertDialog.Builder(activity)
				.setTitle(activity.getString(R.string.app_name) + "提示")
				.setMessage(message).setPositiveButton("确定", null).show();
	}

	public static void showAlert(Activity activity, String message,
			AlertDialog.OnClickListener okClickListener) {
		if (activity.isFinishing())
			return;
		new AlertDialog.Builder(activity)
				.setTitle(
						activity.getResources().getString(R.string.app_name)
								+ "提示").setMessage(message)
				.setPositiveButton("确定", okClickListener).show();
	}

	public static void showAlertOkCancel(Activity activity, String message,
			AlertDialog.OnClickListener okClickListener,
			AlertDialog.OnClickListener cancelClickListener) {
		if (activity.isFinishing())
			return;
		new AlertDialog.Builder(activity)
				.setTitle(activity.getString(R.string.app_name) + "提示")
				.setMessage(message).setPositiveButton("确定", okClickListener)
	...
	public static String getFlurryKey(Context context) {
		if (!TextUtils.isEmpty(flurryKey)) {
			AzkHelper.showLog("channel", "flurryKey: " + flurryKey);
			return flurryKey;
		}
		String ret = null;
		...
		if (TextUtils.isEmpty(ret)) {
			...
			AzkHelper.showLog("channel", "读取manifest里的FLURRY_APPKEY: " + ret);
		}
		flurryKey = ret;
		return ret;
	}
	...
	public static boolean isSinaInstalled(Context context) {
		...
		return AzkHelper.isIntentAvailable(context, intent);
	}
